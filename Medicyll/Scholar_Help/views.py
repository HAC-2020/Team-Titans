...
def inc_service(request):
    return render(request, 'insurance.html', {})

def inc_doc(request):
    return render(request, 'inc_doc.html', {})


def inc_view(request):
    return render(request, 'view_inc.html', {"url":docurl})
...
def userapply(req):
    ...
    extractedInformation = pytesseract.image_to_string('test.jpg')
   
    extractedInformation=extractedInformation.replace('\n','*121*')
    import re
    found = " "
    m = re.search('Patient Value(.+?)Urea', extractedInformation)
    print("----")
    if m:
        found = m.group(1)
        print("----")
        found=found.replace('*121*','\n')
        print(found, " matches")

    return render(req, 'viewack.html',
                  {'summary':found,'scheme':
